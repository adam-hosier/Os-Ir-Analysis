...
#style = 'diff'
style = 'diff'
r=4                                #number of data points used in analysis
...
ftranloc = r'.'
#Desktop Files
wavefile = str(ftranloc + '\\Calibration6bOsIrWavelengthCal6b.csv')          ###Adams calibration, but seperated for each individual spectra
wavefile2 = str(ftranloc + '\\Calibration6bOsIrWavelengthCal6b.csv')     ###Adam's calibration (cal set basis)
...
redDatatab = str(ftranloc + '\\OsIr_NucChargeRadius_ReducedDataTable3.csv')
...
def Histogram(res, bins, bin_range, num):
    ...
    #print(np.max(np.abs(res)), res, bins, bin_range)
    #print(freq, yuncert, uncerterr)
...
def OutlierDet(osna, Oserr, osnatime, irna, Irerr, irnatime, poly, crit):       #single point removal
    ...
    for i in range(np.shape(irna)[0]):
        ...
        IHis = Histogram(resiI, bins=bins, bin_range=(np.average(np.abs(resiI[np.abs(resiI)<5]))*1.5), num=newfit['num'])
...
for v in range(1, 11):
    ...
    resIrtest = modtestIr.fit(np.array(testIr), params=paramstestIr, x=np.array(pIr), max_nfev=None, nan_policy='omit')
    ...
    resOstest = modtestOs.fit(np.array(testOs), params=paramstestOs, x=np.array(pOs), max_nfev=None, nan_policy='omit')
    ...
    for k in range(np.shape(df)[1]):        #Ir individual spectra(Na-like)
        ...
        if int(df2.iloc[2,k][3:]) == int(v):
            ...
            resultcalir = modcalIr.fit(np.array(irfit), params=params, x=np.array(irpix), weights=None, nan_policy='omit', max_nfev=None)
    ...
    for b in range(np.shape(dfa)[1]):           #Os individual spectra Na-like
        ...
        if int(df2a.iloc[2,b][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalos = modcalOs.fit(osfit, params=params, x=np.array(ospix), weights=None, nan_policy='omit', max_nfev=None)
    ...
    for k in range(np.shape(df)[1]):        #Ir individual spectra(Na-like) 2nd order
        ...
        if int(df2.iloc[2,k][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalir = modcalIr.fit(irfit, params=params, x=np.array(irpix), weights=None, nan_policy='omit', max_nfev=None)
            params.update(resultcalir.params)
    ...
    result = mod.fit(np.array(yfit), params=params, x=np.array(xfit), weights=None, nan_policy='omit', max_nfev=None)
    ...
    result2 = mod.fit(np.array(yfit2), params=params2, x=np.array(xfit2), weights=None, nan_policy='omit', max_nfev=None)
    ...
    result = mod3.fit(np.array(yfit3), params=params, x=np.array(xfit3), weights=None, nan_policy='omit', max_nfev=None)
    params.update(result.params)
    ...
    result2 = mod4.fit(np.array(yfit4), params=params2, x=np.array(xfit4), weights=None, nan_policy='omit', max_nfev=None)
    params2.update(result2.params)
...
#print('shape before', osmg.shape, irmg.shape)
#print(osmg, irmg)

#osmg_bef, irmg_bef = osmg, irmg

""" START MONTE CARLO """
osmg = rng.normal(osmg.astype('float'), osmge.astype('float'))
irmg = rng.normal(irmg, irmge)
""" END MONTE CARLO """

# print('shape before', osmg.shape, irmg.shape)
# print(osmg - osmg_bef, irmg - irmg_bef)
...
dispuncE = 167.5 * (dispunc / 7.37185)
totalrelunc = np.sqrt(ntotalunce**2 + dispuncE**2)
print(ndE, ' +/- ', totalrelunc, ' eV')

with open('montecarlo.csv','a') as fd:
    fd.write(f'\n{absendiff}, {absendiffe}, {ndE}, {totalrelunc}')
