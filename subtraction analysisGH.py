...
#Desktop Files
wavefile = '.\\Calibration6bOsIrWavelengthCal6b.csv'         ###Adams calibration, but seperated for each individual spectra
wavefile2 = '.\\Calibration6bOsIrWavelengthCal6b.csv'     ###Adam's calibration (cal set basis)
#wavefile2 = str(ftranloc + '\\OsIrWavelengthCalSam.csv')    ###Sam's calibration (cal set basis)
sysfile = '.\\Calibration6bSystematic_Uncertainty.csv'
visualbadfiles = '.\\badshit.csv'
redDatatab = '.\\OsIr_NucChargeRadius_ReducedDataTable3.csv'         #was 2
#specNumtab = str('ftranloc + '\\OsIr_NucChargeRadius_SpectrumNo02.csv')
specNumtab = '.\\OsIr_NucChargeRadius_SpectrumNo.csv'
dropTab = '.\\OsIr_NucChargeRadius_SpectrumNo2.csv'
...
def Histogram(res, bins, bin_range, num):
    ...
    errfit = mod1.fit(freq, params, x=yuncert, weights = uncerterr, nan_policy='omit')  #weights = uncerterr
...
def quadgfit(x, y, r, c, num):
    ...
    res = mod.fit(sig, params=params, x=wave, weights = np.sqrt(np.abs(sig)))
...
def linfit(x, y):
    ...
    res = mod.fit(y, x=x, params=params, weights = None)
    params.update(res.params)
...
def gfit(x, y, yerr):
    ...
    res = mod.fit(y, params=params, x=x, weights=np.sqrt(np.abs(y)), max_nfev=None)
    params.update(res.params)
...
def bgpolyfit(x, y):
    ...
    res = mod.fit(y, params=params, x=x, weights=None, )
    params.update(res.params)
...
def testdgfit(x, y, num):
    ...
    res = mod.fit(y, params=params, x=x, weights =np.sqrt(np.abs(y)))
    params.update(res.params)
...
for v in range(1, 11):
    ...
    resIrtest = modtestIr.fit(testIr, params=paramstestIr, x=pIr, max_nfev=None)
    ...
    resOstest = modtestOs.fit(testOs, params=paramstestOs, x=pOs, max_nfev=None)
    ...
    naresIrtest = namodtestIr.fit(natestIr, params=naparamstestIr, x=napIr, max_nfev=None)
    ...
    naresOstest = namodtestOs.fit(natestOs, params=naparamstestOs, x=napOs, max_nfev=None)
    ...
    mgresIrtest = mgmodtestIr.fit(mgtestIr, params=mgparamstestIr, x=mgpIr, max_nfev=None)
    ...
    mgresOstest = mgmodtestOs.fit(mgtestOs, params=mgparamstestOs, x=mgpOs, max_nfev=None)
    ...
    naresIrtest2 = namodtestIr.fit(natestIr2, params=naparamstestIr2, x=napIr2, max_nfev=None)
    ...
    naresOstest2 = namodtestOs.fit(natestOs2, params=naparamstestOs2, x=napOs2, max_nfev=None)
    ...
    for k in range(np.shape(df)[1]):        #Ir individual spectra(Na-like)
        ...
        if int(df2.iloc[2,k][3:]) == int(v):
            ...
            resultcalir = modcalIr.fit(irfit2, params=params, x=irpix, weights=1/np.sqrt(np.abs(irfit2)), max_nfev=None)
            ...
            resultir = modIr.fit(irfit2, params=params, x=irwave, max_nfev = None, weights=np.sqrt(np.abs(irfit2)))
            #resultir = modIr.fit(irfit2, params=params, x=irwave, max_nfev = None, weights=np.sqrt(np.abs(pweight)))
    ...
    for k in range(np.shape(df)[1]):        #Ir individual spectra(Mg-like)
        ...
        if int(df2.iloc[2,k][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalir = modcalIr.fit(irfit2, params=params, x=irpix, weights=np.sqrt(np.abs(irfit2)), max_nfev=None)
            params.update(resultcalir.params)
            ...
            resultir = modIr.fit(irfit2, params=params, x=irwave, max_nfev = None, weights=np.sqrt(np.abs(irfit2))/np.sum(np.sqrt(np.abs(irfit2))))
            #resultir = modIr.fit(irfit2, params=params, x=irwave, max_nfev = None, weights=np.abs(pweight)/np.sqrt(np.abs(pweight)))
    ...
    for b in range(np.shape(dfa)[1]):           #Os individual spectra Na-like
        ...
        if int(df2a.iloc[2,b][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalos = modcalOs.fit(osfit2, params=params, x=ospix, weights=np.sqrt(np.abs(osfit2)), max_nfev=None)
            #resultcalos = modcalOs.fit(osfit2, params=params, x=ospix, weights=1/np.sqrt(np.abs(pweight)), max_nfev=None)
            ...
            resultos = modOs.fit(osfit2, params=params, x=oswave, weights=np.abs(osfit2)/np.sqrt(np.abs(osfit2)), max_nfev=None)
    ...
    for b in range(np.shape(dfa)[1]):           #Os individual spectra Mg-like
        ...
        if int(df2a.iloc[2,b][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalos = modcalOs.fit(osfit, params=params, x=ospix, weights=None, max_nfev=None)
            ...
            #resultos = modOs.fit(osfit2, params=params, x=oswave, weights=np.abs(pweight)/np.sqrt(np.abs(pweight)), max_nfev=None)
            resultos = modOs.fit(osfit2, params=params, x=oswave, weights=np.abs(osfit2)/np.sqrt(np.abs(osfit2)), max_nfev=None)
    ...
    for k in range(np.shape(df)[1]):        #Ir individual spectra(Na-like) 2nd order
        ...
        if int(df2.iloc[2,k][3:]) == int(v):
            ...
            params.add('mu', value = c)
            params.add('sig', value = 0.5)
            resultcalir = modcalIr.fit(irphotc, params=params, x=irpix, weights=None, max_nfev=None)
            params.update(resultcalir.params)
            ...
            resultir = modIr.fit(irphotc, params=params, x=irwave, max_nfev = None, weights=irphot/np.sqrt(irphot))
            params.update(resultir.params)
            xplot = np.linspace(np.min(irwave), np.max(irwave), num=1000)
            yplot = modIr.eval(params=params, x=xplot)
    ...
    for b in range(np.shape(dfa)[1]):           #Os individual spectra Na-like 2nd order
        ...
        if int(df2a.iloc[2,b][3:]) == int(v):
            ...
            resultos = modOs.fit(osphot, params=params, x=oswave, weights=osphot/np.sqrt(osphot), max_nfev=None)
            params.update(resultos.params)
    ...
    result = mod.fit(yfit, params=params, x=xfit, weights=None, max_nfev=None)
    ...
    result2 = mod.fit(yfit2, params=params2, x=xfit2, weights=None, max_nfev=None)
    ...
    result = mod3.fit(yfit3, params=params, x=xfit3, weights=None, max_nfev=None)
    params.update(result.params)
    ...
    result2 = mod4.fit(yfit4, params=params2, x=xfit4, weights=None, max_nfev=None)
    params2.update(result2.params)
...
""" START MONTE CARLO """
osna = rng.normal(osna.astype('float'), osnae.astype('float'))
irna = rng.normal(irna.astype('float'), irnae.astype('float'))
""" END MONTE CARLO """
...
""" START MONTE CARLO """
osmg = rng.normal(osmg.astype('float'), osmge.astype('float'))
irmg = rng.normal(irmg.astype('float'), irmge.astype('float'))
""" END MONTE CARLO """
...
""" START MONTE CARLO """
osna2 = rng.normal(osna2.astype('float'), osnae2.astype('float'))
irna2 = rng.normal(irna2.astype('float'), irnae2.astype('float'))
""" END MONTE CARLO """
...
""" START MONTE CARLO """
osnacen = rng.normal(osnacen.astype('float'), osnacenu.astype('float'))
irnacen = rng.normal(irnacen.astype('float'), irnacenu.astype('float'))
osmgcen = rng.normal(osmgcen.astype('float'), osmgcenu.astype('float'))
irmgcen = rng.normal(irmgcen.astype('float'), irmgcenu.astype('float'))
""" END MONTE CARLO """
...
""" START MONTE CARLO """
irna2c = rng.normal(irna2c.astype('float'), irnae2c.astype('float'))
osna2c = rng.normal(osna2c.astype('float'), osnae2c.astype('float'))
""" END MONTE CARLO """
...
nirmge = np.delete(nirmge, np.where(nirmge > 0.05))
...
OsNaEnergy = JeV* hp * cs / (np.array(nOsNaavg)* 10**(-9))
IrNaEnergy = JeV * hp * cs / (np.array(nIrNaavg)* 10**(-9))

OsNaEnergye = nOsNaavge * (OsNaEnergy/nOsNaavg)
IrNaEnergye = nIrNaavge * (IrNaEnergy/nIrNaavg)
...
print(' ')
print(' ')
print('Relative difference determination: ')
dispunc = np.average(dispcalunc)
totalrelunc = np.sqrt(ntotalunce**2 + dispunc**2)
print(ndE, ' +/- ', totalrelunc, ' eV')
