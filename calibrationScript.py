...
#df.drop(df.columns[df.columns.str.contains('EUV037', case=False)], axis=1, inplace=True)
df.drop(df.columns[df.columns.str.contains('5p6', case=False)], axis=1, inplace=True)
...
df2.drop(df2.columns[df2.columns.str.contains('EUV004_Ne_3p99keV_1', case=False)], axis=1, inplace=True)
#df2.drop(df2.columns[df2.columns.str.contains('EUV085', case=False)], axis=1, inplace=True)
#df2.drop(df2.columns[df2.columns.str.contains('EUV037', case=False)], axis=1, inplace=True)
...
dfb.drop(dfb.columns[dfb.columns.str.contains('0p9', case=False)], axis=1, inplace=True)
#dfb.drop(dfb.columns[dfb.columns.str.contains('EUV086', case=False)], axis=1, inplace=True)
#dfb.drop(dfb.columns[dfb.columns.str.contains('EUV040', case=False)], axis=1, inplace=True)
...
df2b.drop(df2b.columns[df2b.columns.str.contains('18p12', case=False)], axis=1, inplace=True)
#df2b.drop(df2b.columns[df2b.columns.str.contains('EUV086', case=False)], axis=1, inplace=True)
#df2b.drop(df2b.columns[df2b.columns.str.contains('EUV040', case=False)], axis=1, inplace=True)
...
cbg = np.array([96, 209, 270, 367, 447, 1225, 1248, 1297, 1335, 1527, 1766, 1775])


wlen = np.array([8.80929, 9.7502, 9.82141, 10.6138, 11.1136, 11.6691, 12.7676, 14.3314, 14.7138, 17.61858, 19.5004])
wlenbg = np.array([6.2909, 6.6623, 15.0101])

wlenerr = np.array([0.00014, 0.0004, 0.00021, 0.0005, 0.0018, 0.0005, 0.0007, 0.0007, 0.0007, 0.00028, 0.0008])
wlenerrbg = np.array([0.0025, 0.0007, 0.0005])

c = np.array([763, 890, 900, 1004, 1067, 1137, 1270, 1451, 1494, 1807, 1999])
cbg = np.array([388, 447, 1527])
'''
#################################################
####Dr. Takacs' and my lines
'''
wlen = np.array([6.73832, 8.80929,9.7502,10.6138,11.1136,11.6691, 14.3314, 15.452,17.7164,19.5004])
wlenbg = np.array([4.5788,4.761,6.2909,6.6623, 13.3256])

wlenerr = np.array([0.00012, 0.00014,0.0004,0.0005,0.0018,0.0005, 0.0007,0.0007,0.0006,0.0008])
wlenerrbg = np.array([0.001,0.001,0.0025,0.0007, 0.001])

c = np.array([458, 763,890,1004,1067,1137, 1451,1576,1818,1999])
cbg = np.array([96,130,388,447, 1336])
'''
#################################################
...
channelbg = np.zeros([len(cbg), num_colsbg-m], dtype=object)
channelerrbg = np.zeros([len(cbg), num_colsbg-m], dtype=object)
residbg = np.zeros([len(cbg), num_colsbg-m])
snbg = np.zeros([len(cbg), num_colsbg-m])
...
for k in range(m, num_colsbg):    
    ...
    for l in range(len(cbg)):
        ...
        residbg[l,k] = cbg[l] - Gaussfit['mu']

        if Gaussfit['mu'] > cbg[l] + lim or Gaussfit['mu'] < cbg[l] - lim:
            channelbg[l,k] = 'nan'
            channelerrbg[l,k] = 'nan'
            #channelbg[l,k] = np.nan
            #channelerrbg[l,k] = np.nan
            print('nanbg')
###
...
def pixelcal(cen, cenerr, wlen, wlenerr, sys):#in global loop 
    ...
    def pfit(K, x):
        return K[0] + K[1]*x + K[2]*x**2 + K[3]*x**3
    cen = np.array(cen)
    ...
    nvar = len(out.beta)
    ndata = len(cen)
    residarr = (wlen - test(cen,out.beta[0], out.beta[1], out.beta[2], out.beta[3]))/ (np.sqrt((wlenerr)**2 + (testd(cen, out.beta[1], out.beta[2], out.beta[3], cenerr))**2 + sys**2))
    ...
    if badcount > 3: 
        ...
        polyc['K0'] = out.beta[0]
        polyc['K0e'] = out.sd_beta[0]
        polyc['K1'] = out.beta[1]
        polyc['K1e'] = out.sd_beta[1]
        polyc['K2'] = out.beta[2]
        polyc['K2e'] = out.sd_beta[2]
        polyc['K3'] = out.beta[3]
        polyc['K3e'] = out.sd_beta[3]
        polyc['red_chi_sq'] = red_chi_sq
        #print(out.res_var)
        #print(red_chi_sq)
        #print('~~~~')
...
for i in range(num_cols):
    ...
    #arr = np.delete(arr, np.where( np.bitwise_and( (arr[:,0]==np.nan), arr[:,0]=='nan'))[0], 0)
    #print(arr)
    #print(arr[-1,0])
    #print(type(arr[-1,0]))
    #print(np.any(arr==np.nan))
